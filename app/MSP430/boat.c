...
void boat_loop(unsigned long timestamp, double heading) {
    ...
    int BaseSpeed = 90;                     // Motor base speed
    ...
    if (first_time ==1) {
        servo1_move_to_angle(90); // 0 for dual
    ...
    if (((calculateDifferenceBetweenAngles(heading, 90)) > 0.0) && (start==0)) {
        ...
        servo2_move_to_angle(BaseSpeed);  // range 0-180
        //servo1_move_to_angle(BaseSpeed); // range 0-180
